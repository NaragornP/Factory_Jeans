...
if ($num2>$num) {
	...
	echo '<META HTTP-EQUIV="Refresh" CONTENT="5;URL=zip.php">';
} else {

	// $objConnect = mysql_connect($servername,$username,$password) or die("Error Connect to Database");
	// $objDB = mysql_select_db($dbname);
	// $strSQL = "INSERT INTO zip_out ";
	// $strSQL .="(zo_code,zo_number,zo_date) VALUES ('".$code."','".$num2."','".$date."')";
	// $objQuery = mysql_query($strSQL);	

	$numUpdate = $num - $num2;
	// echo $yardUpdate;

	$sql = "UPDATE zip SET z_number='".$numUpdate."' WHERE z_code = '$code' ";
		if ($conn->query($sql) === TRUE) {
		    // echo "<br>Record updated successfully";
		} else {
		    // echo "Error updating record: " . $conn->error;
		}
	$sql = "INSERT INTO equip_transection (et_zip, et_date_out, et_number_out,zip_id) VALUES ('".$code."','".$date."','".$num2."',1)";
		if ($conn->query($sql) === TRUE) {
	    // echo "<br>Record updated successfully";
		} else {
		    // echo "Error updating record: " . $conn->error;
