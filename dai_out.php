...
if ($num2>$num) {
	...
	echo '<META HTTP-EQUIV="Refresh" CONTENT="5;URL=dai.php">';
} else {

	// $objConnect = mysql_connect($servername,$username,$password) or die("Error Connect to Database");
	// $objDB = mysql_select_db($dbname);
	// $strSQL = "INSERT INTO dai_out ";
	// $strSQL .="(do_code,do_number,do_date) VALUES ('".$code."','".$num2."','".$date."')";
	...
	$sql = "UPDATE dai SET k_number='".$numUpdate."' WHERE k_code = '$code' ";
		if ($conn->query($sql) === TRUE) {
		    // echo "<br>Record updated successfully";
		} else {
		    // echo "Error updating record: " . $conn->error;
		}
	$sql = "INSERT INTO equip_transection (et_dai, et_date_out, et_number_out,dai_id) VALUES ('".$code."','".$date."','".$num2."',1)";
		if ($conn->query($sql) === TRUE) {
	    // echo "<br>Record updated successfully";
		} else {
		    // echo "Error updating record: " . $conn->error;
